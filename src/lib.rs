...
impl<'a> Window<'a> {
    ...
    pub fn apply_snapshot(&mut self, snapshot:&Snapshot, x: i32,y: i32){
        self.apply_snapshot_sub(snapshot, x, y, TRANSPARENT);//sub
    }
    /// Write a snapshot at a point, saturating substracting a color to its fg and bg
    pub fn apply_snapshot_sub(&mut self, snapshot: &Snapshot, x: i32, y: i32,sub:Col) {
        //u32//u32
        let bx = x;
        let by = y;
        for x in bx..bx + snapshot.size.0 as i32 {
            for y in by..by + snapshot.size.1 as i32 {
                if x - bx < snapshot.size.0 as i32 && y - by < snapshot.size.1 as i32 {
                    ...
                    let fg = (
                        snapshot.fg[idx * 4].saturating_sub(sub.0),//rhs
                        snapshot.fg[idx * 4 + 1].saturating_sub(sub.1),
                        snapshot.fg[idx * 4 + 2].saturating_sub(sub.2),
                        snapshot.fg[idx * 4 + 3].saturating_sub(sub.3),
                    );
                    self.set_fg_at(x, y, fg);
                    let bg = (
                        snapshot.bg[idx * 4].saturating_sub(sub.0),
                        snapshot.bg[idx * 4 + 1].saturating_sub(sub.1),
                        snapshot.bg[idx * 4 + 2].saturating_sub(sub.2),
                        snapshot.bg[idx * 4 + 3].saturating_sub(sub.3),
                    );
                    self.set_bg_at(x, y, bg);
                }
            }
    ...
    T: Yarl2Game,
{
    ...
    fn window_event(
        ...
        //event_loop.set
        self.game.event(&event, &mut self.window,&self.keyboard);
        match event {
            ...
            winit::event::WindowEvent::CursorMoved {
                ...
                self.keyboard.mouse_position_float = (x as f32, y as f32);
            }
...
pub trait Yarl2Game {
    ...
    fn event(&mut self, _event: &le_winit::event::WindowEvent, _window: &mut Window,_keyboard:&NiceKeyboard) {
        //I
    }
}
impl Yarl2Game for () {
...
pub struct NiceKeyboard {
    ...
    /// The mouse position as a float
    pub mouse_position_float: (f32, f32)
}
/*
impl NiceKeyboard{
